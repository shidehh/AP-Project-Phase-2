...
def sign_up(request):
    if request.method == 'POST':
        ...
        user = User.objects.create_user(username=national_code, password=password)
        patient = PatientInfo.objects.create(user=user, patient_name=name, patient_contact_info=contact_info)
...
def select_clinic_capacity_info(request, clinic_id):
    if request.method == 'GET':
        try:
            ...
            return JsonResponse({"message": "Clinic not found."})
    else:
        return JsonResponse({"message": "Invalid request method."})
