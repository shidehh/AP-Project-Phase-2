...
def reserve_appointment(request):
    if request.method == 'POST':
        ...
        # Make a POST request to the /reserve endpoint of the Flask API
        url = 'http://127.0.0.1:5000/reserve'
        data = {'id': clinic.id, 'reserved': number_of_reservations}
        response = requests.post(url, json=data)
        result = response.json()
...
def cancel_appointment(request):
    if request.method == 'POST':
        ...
        # Make a POST request to the /reserve endpoint of the Flask API to cancel the reservation
        url = 'http://127.0.0.1:5000/reserve'
        data = {'id': clinic.id, 'reserved': -number_of_cancellations}  # Use a negative number to indicate cancellation
        response = requests.post(url, json=data)
        result = response.json()
        ...
        if result['success']:
            ...
            if user_type == 'patient':
                return render(request, 'patient_reserved_appointment.html', {"message": "Failed to cancel appointment"})
            else:
                return render(request, 'secretary_cancel_appointment.html', {"message": "Failed to cancel appointment"})
    else:
        return render(request, 'patient_reserved_appointment.html', {"message": "Invalid request method."})
...
def increase_capacity(request):
    if request.method == 'POST':
        ...
        # Get the clinic based on the provided ID
        clinic = Clinic.objects.get(id=clinic_id)

        # Make a POST request to the /reserve endpoint of the Flask API to increase the capacity
        url = 'http://127.0.0.1:5000/reserve'
        data = {'id': clinic.id, 'reserved': -int(increase_amount)}  # Use a negative number to indicate increasing capacity
        response = requests.post(url, json=data)
        result = response.json()
        ...
        if result['success']:
            ...
            return render(request, 'secretary_increase_capacity.html', {"message": "Failed to increase capacity"})
    else:
        return render(request, 'secretary_increase_capacity.html', {"message": "Invalid request method."})'''